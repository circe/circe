...
trait ProductDecoder[L <: HList] {
  def apply(history: List[HistoryOp], js: List[HCursor]): Xor[DecodingFailure, L]
}

object ProductDecoder {
  implicit val productDecodeHNil: ProductDecoder[HNil] = new ProductDecoder[HNil] {
    def apply(history: List[HistoryOp], js: List[HCursor]): Xor[DecodingFailure, HNil] =
  ...
  implicit def productDecodeHList[H, T <: HList](implicit
    ...
    def apply(history: List[HistoryOp], js: List[HCursor]): Xor[DecodingFailure, H :: T] = js match {
