...
class BiggerDecimalSuite extends FlatSpec with GeneratorDrivenPropertyChecks {
  ...
  "fromDouble" should "round-trip Double values" in {
    forAll { (value: Double) =>
  ...
  "fromLong" should "round-trip Long values" in {
    forAll { (value: Long) =>
  ...
  "fromBigDecimal" should "round-trip BigDecimal values" in {
  ...
  "fromBigInteger" should "round-trip BigInteger values" in {
    forAll { (value: BigInt) =>
