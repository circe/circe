...
import io.jfc.{ CursorOp, Decode, DecodeFailure, HCursor, Json }
import shapeless._

trait ArrayDecode[L <: HList] {
  def apply(history: List[CursorOp], js: List[HCursor]): Xor[DecodeFailure, L]
}

object ArrayDecode {
  implicit val arrayDecodeHNil: ArrayDecode[HNil] = new ArrayDecode[HNil] {
    def apply(history: List[CursorOp], js: List[HCursor]): Xor[DecodeFailure, HNil] =
  ...
  implicit def arrayDecodeHList[H, T <: HList](implicit
    ...
    def apply(history: List[CursorOp], js: List[HCursor]): Xor[DecodeFailure, H :: T] = js match {
