...
trait Decoder[A] extends Serializable {
  ...
  /**
    ...
    */
  def tryDecode(c: ACursor): Decoder.Result[A] = if (c.succeeded) apply(c.any)
  else Xor.left(
  ...
  def productAcumulator[B](x: Decoder[B]):
  (HCursor, HCursor) => AccumulatingDecoder.Result[(A, B)] =
...
object Decoder extends TupleDecoders with LowPriorityDecoders {
  ...
  implicit def decodeOption[A](implicit d: Decoder[A]): Decoder[Option[A]] =
    withReattempt(c =>
      if (c.succeeded) {
        if (c.any.focus.isNull) rightNone
        else d(c.any) match {
