...
final object semiauto {
  ...
  final def deriveDecoderWithIso[B, A](implicit decoder: Decoder[A], iso: Iso[B, A], e: DummyImplicit): Decoder[B] =
    decoder.map(iso.reverseGet)

  final def deriveEncoderWithIso[B, A](implicit encoder: Encoder[A], iso: Iso[B, A], e: DummyImplicit): Encoder[B] =
