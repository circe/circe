...
private[circe] trait EncoderDerivation {
  inline final def derived[A](given A: Mirror.Of[A]): Encoder.AsObject[A] =
    new DerivedEncoder[A](Derivation.summonEncoders[A.MirroredElemTypes])
...
private[circe] trait DecoderDerivation {
  inline final def derived[A](given A: Mirror.Of[A]): Decoder[A] =
    new DerivedDecoder[A](Derivation.summonDecoders[A.MirroredElemTypes])
        with DerivedInstance[A](
          constValue[A.MirroredLabel],
          Derivation.summonLabels[A.MirroredElemLabels]
        ) {
...
private[circe] trait CodecDerivation {
  inline final def derived[A](given A: Mirror.Of[A]): Codec.AsObject[A] =
    new Codec.AsObject[A]
        with DerivedDecoder[A](Derivation.summonDecoders[A.MirroredElemTypes])
        with DerivedEncoder[A](Derivation.summonEncoders[A.MirroredElemTypes])
        with DerivedInstance[A](
          constValue[A.MirroredLabel],
          Derivation.summonLabels[A.MirroredElemLabels]
        ) {
...
private[circe] trait DerivedEncoder[A](
  private[this] val elemEncoders: Array[Encoder[_]]
) extends DerivedInstance[A] with Encoder.AsObject[A] {
...
private[circe] trait DerivedDecoder[A](
  private[this] val elemDecoders: Array[Decoder[_]]
) extends DerivedInstance[A] with Decoder[A] {
